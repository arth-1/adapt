...
export function SendMoneyOptions({ onBack, language }: SendMoneyOptionsProps) {
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-cyan-400 to-blue-500 p-6 relative">
      {/* Header */}
      <div className="flex justify-between items-center mb-8">
        <p className="text-white text-xl font-semibold">{t.sendMoney}</p>
        <button
          onClick={onBack}
      ...
      <div className="space-y-6">
        ...
        <button className="w-full bg-white rounded-3xl p-8 flex flex-col items-center gap-4 shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95">
          ...
          <div className="text-center">
            <p className="text-gray-900 font-semibold text-lg mb-2">{t.scanQR}</p>
            <p className="text-gray-600 text-sm">{t.scanDesc}</p>
          </div>
          <div className="flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-full">
            <Scan className="w-4 h-4 text-blue-600" />
            <span className="text-blue-600 text-sm font-medium">Quick & Secure</span>
          </div>
        </button>
        ...
        <button className="w-full bg-white rounded-3xl p-8 flex flex-col items-center gap-4 shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95">
          ...
          <div className="text-center">
            <p className="text-gray-900 font-semibold text-lg mb-2">{t.phoneNumber}</p>
          ...
          <div className="flex items-center gap-2 bg-teal-50 px-4 py-2 rounded-full">
            ...
            <span className="text-teal-600 text-sm font-medium">Easy & Fast</span>
          </div>
        </button>
      </div>
